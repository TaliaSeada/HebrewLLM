...
def objective(trial):
    ...
    dim_feedforward = trial.suggest_int('dim_feedforward', 256, 4096)
...
if __name__ == '__main__':
    ...
    print("--------- CHECK ----------")
    max_length = 15
    prompt = "I love to go to the beach"
    ...
    translator_inputs = translator_tokenizer(generated_text, return_tensors="pt", padding='max_length', truncation=True,
                                          max_length=max_length).to(device)
    decoder_start_token_id = translator_tokenizer.pad_token_id
    decoder_input_ids = torch.full((translator_inputs.input_ids.size(0), 1), decoder_start_token_id,
                                dtype=torch.long).to(device)
    translator_outputs = translator_model(input_ids=translator_inputs.input_ids,
                                       attention_mask=translator_inputs.attention_mask,
                                       decoder_input_ids=decoder_input_ids, output_hidden_states=True)
    translator_hidden_state = translator_outputs.encoder_hidden_states[translator_layer].detach()
